(() => {
  ...
  async function add(event){event.preventDefault();try{const created=await api('/api/admin/integrations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:$('integration-name').value.trim(),provider:$('integration-provider').value,api_key:$('integration-api-key').value.trim(),model:$('integration-model').value.trim()})});$('integration-api-key').value='';message('API enregistrée. Test réel en cours…',true);if(created?.id){try{const tested=await api(`/api/admin/integrations/${created.id}/test`,{method:'POST'});message(tested.message||'API opérationnelle et activée.',true)}catch(testError){message('API enregistrée mais inutilisable : '+testError.message)}}await load();window.PiGameStudioAdmin?.load?.()}catch(e){message(e.message)}}
